...
@section('content')
    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header">
                <h5>Event Calendar Management</h5>
            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            table = $("#dataTable").DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{URL::to('/mcalendarajax')}}",
                columns: [
                    {data: 'checkbox', name: 'checkbox', orderable: false, searchable: false},
                    {data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false},
                    {data: 'event_title', name: 'u.event_title'},
                    {data: 'updated_at', name: 'u.updated_at', orderable: false},
        ...
        $(document).on("click", ".btn-activate", function () {
            ...
            $('#actionForm').attr('action', '{{url('/mcalendar/toggle')}}');
            $('#actionForm').submit();
        });
        @if($update=='X')
        $(document).on("click", ".btn-edit", function () {
            var userid = $(this).attr('userid');
            window.location.href = '/mcalendar/edit/' + userid;
        });
        @endif
        ...
        $(document).on("click", "#actDelete", function () {
            ...
            if (ids != "") {
                $('#state_param').val(2);
                $('#userid_param').val(ids);
                $('#actionForm').attr('action', '{{url('/mcalendar/destroy')}}');
