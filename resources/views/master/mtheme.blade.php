...
@section('content')
    ...
    <script>
        ...
        $(document).on("click", ".btn-activate", function () {
            ...
            $('#actionForm').attr('action', '{{url('/mtheme/toggle')}}');
            $('#actionForm').submit();
        });
        @if($update=='X')
        $(document).on("click", ".btn-edit", function () {
            var userid = $(this).attr('userid');
            window.location.href = '/mtheme/edit/' + userid;
        });
        @endif
        ...
        $(document).on("click", "#actDelete", function () {
            ...
            if (ids != "") {
                $('#state_param').val(2);
                $('#userid_param').val(ids);
                $('#actionForm').attr('action', '{{url('/mtheme/destroy')}}');
