...
class CrudGeneratorCommand extends Command
{
    ...
    public function handle()
    {
        ...
        if (substr($name, 0, 1) == 'M' || substr($name, 0, 1) == 'T') {
            ...
            File::append(base_path('routes/web.php'), 'Route::post(\'/' . strtolower($name) . "/update/{id}','{$name}Controller@webUpdate');");
            File::append(base_path('routes/web.php'), 'Route::post(\'/' . strtolower($name) . "/destroy/{id}','{$name}Controller@webDestroy');");
